...
export const TrainDetail: React.FC<TrainDetailProps> = ({ train, onNext, onPrev }) => {
  ...
  // Reset/Sync form when train changes
  useEffect(() => {
    setFormState({
        actualTask: train.taskType, // Default to planned
        staff: ['Juan Pérez', 'Ana García'], // Mock default staff
        startTime: '22:00',
        endTime: '06:00',
        hasIncident: false,
        incidentDescription: ''
    ...
    <section className="flex-1 flex flex-col bg-background-light relative min-w-[500px] h-full">
      ...
      <header className="h-16 px-8 flex items-center justify-between bg-surface border-b border-border shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 leading-none">Validación: Tren {train.name}</h1>
            <p className="text-xs text-slate-500 mt-1">Last updated by Supervisor A. at 14:00</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={onPrev} className="flex items-center justify-center size-9 rounded-lg border border-border text-slate-500 hover:bg-slate-50 hover:text-slate-800 transition-colors" title="Previous Train">
            <span className="material-symbols-outlined">chevron_left</span>
          </button>
          <button onClick={onNext} className="flex items-center justify-center size-9 rounded-lg border border-border text-slate-500 hover:bg-slate-50 hover:text-slate-800 transition-colors" title="Next Train">
            <span className="material-symbols-outlined">chevron_right</span>
          </button>
          <button className="flex items-center justify-center size-9 rounded-lg border border-border text-slate-500 hover:bg-slate-50 hover:text-slate-800 transition-colors ml-2" title="More Options">
          ...
          <div className="bg-surface rounded-xl shadow-sm border border-border p-6">
            ...
            <div className="grid grid-cols-2 gap-8">
              ...
              <div className="bg-slate-50 rounded-lg p-4 border border-slate-100">
                <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Tarea Planificada</label>
              ...
              {/* Actual (Editable) */}
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Tarea Realizada</label>
                <div className="relative">
                  <select 
                    value={formState.actualTask}
                    onChange={(e) => setFormState(prev => ({ ...prev, actualTask: e.target.value }))}
            ...
            <div className="md:col-span-2 bg-surface rounded-xl shadow-sm border border-border p-6">
              ...
              <div className="w-full min-h-[56px] rounded-lg border border-slate-300 bg-white p-2 flex flex-wrap gap-2 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
                {formState.staff.map((member, idx) => (
                    <div key={idx} className="bg-primary/10 text-primary border border-primary/20 px-2 py-1 rounded-md text-sm font-medium flex items-center gap-1">
                        {member}
                        <button onClick={() => removeStaff(idx)} className="hover:bg-primary/20 rounded-full p-0.5 flex items-center justify-center">
            ...
            <div className="flex items-start gap-3">
              <div className="flex items-center h-5">
                <input 
                    ...
                    onChange={(e) => setFormState(prev => ({ ...prev, hasIncident: e.target.checked }))}
                    className="w-5 h-5 rounded border-slate-300 text-primary focus:ring-primary cursor-pointer" 
                />
              </div>
              <div className="flex-1">
                <label htmlFor="incidence" className="text-sm font-semibold text-slate-900 cursor-pointer select-none">Reportar Incidencia</label>
                <p className="text-xs text-slate-500 mt-0.5">Check this box if there were any issues or delays during maintenance.</p>
                
                <div className={`mt-4 transition-all duration-300 ease-in-out ${formState.hasIncident ? 'opacity-100 max-h-40' : 'opacity-50 max-h-40 grayscale pointer-events-none'}`}>
                  <textarea 
                    value={formState.incidentDescription}
                    onChange={(e) => setFormState(prev => ({ ...prev, incidentDescription: e.target.value }))}
                    rows={3} 
      ...
      <footer className="h-20 bg-surface border-t border-border px-8 flex items-center justify-between shrink-0 absolute bottom-0 w-full z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span className="bg-teal-800/40 text-teal-50 px-1.5 py-0.5 rounded text-[10px] font-sans border border-teal-500/30 group-hover:border-teal-400/50 transition-colors">Enter</span>
